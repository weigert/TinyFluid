...
void Field::timestep(){
  ...
  VMAT = -1.0*(vX.asDiagonal()*PDE::XFLUX + vY.asDiagonal()*PDE::YFLUX);

  //Construct Non-Linear Operator from Velocity Field and Other Contributions
  E_MAT = bc.asDiagonal()*(VMAT + viscosity*DMAT);


  P_SOURCE_X.noalias() = bc.cwiseProduct(g.x*(b1+b2).cwiseProduct(VectorXd::Ones(SIZE*SIZE)*v0.x-vX));
  P_SOURCE_Y.noalias() = bc.cwiseProduct(g.y*(b1+b2).cwiseProduct(VectorXd::Ones(SIZE*SIZE)*v0.y-vY));
  P_SOURCE_X.noalias() -= bc.cwiseProduct(PDE::XFLUX*P);
  P_SOURCE_Y.noalias() -= bc.cwiseProduct(PDE::YFLUX*P);

  //Helper Values (Sum over Rows)
  I_MAT = SE - dt*E_MAT;
  A.noalias() = I_MAT*VectorXd::Ones(SIZE*SIZE);
  ...
  //Crank Nicholson
  humidity.noalias() += 0.5*dt*(TRANSPORTMAT_H*humidity + HSOURCE);
  PDE::solver.compute(SE-0.5*dt*TRANSPORTMAT_H);
  humidity = PDE::solver.solve(humidity + 0.5*dt*HSOURCE);

  temperature.noalias() += 0.5*dt*(TRANSPORTMAT_T*temperature + TSOURCE);
  ...
  while(newerr > 1E-4 && pCorr > 1E-6 && !divergence && maxiter){
    ...
    //Crank Nicholson
    humidity += 0.5*dt*(TRANSPORTMAT_H*humidity + HSOURCE);
    PDE::solver.compute(SE-0.5*dt*TRANSPORTMAT_H);
    humidity = PDE::solver.solve(humidity + 0.5*dt*HSOURCE);

    temperature += 0.5*dt*(TRANSPORTMAT_T*temperature + TSOURCE);
    PDE::solver.factorize(SE-0.5*dt*TRANSPORTMAT_T);
    temperature = PDE::solver.solve(temperature + 0.5*dt*TSOURCE);
